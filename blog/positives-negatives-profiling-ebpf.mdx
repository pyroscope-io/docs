...
[ image from https://ebpf.io/what-is-ebpf]
...
While stars are more of a vanity metric, the collective growth is representative of the rapidly growing interest in this space.

[show image of stars over time]
...
The reason that there is this [chasm] between eBPF profilers and non-eBPF profilers is because at each level there
are positives and negatives of profiling at that particular level.

### Pros/Cons of User space profiling (non-eBPF)

#### Pros of User space profiling

- Ability tag application code in flexible way (i.e. tagging spans, controllers, functions, etc.)
- Ability to profile various specific parts of code (i.e. Lambda functions, Test suites, Adhoc scripts, etc.)
- Ability/simplicity to profile other types of data (i.e. Memory profiling, goroutines, etc.)
- Simplicity of using in local development
- Consistency of access to symbols across all languages

#### Cons of User space profiling (eBPF)
...
#### Pros of Kernal profiling

- Ability to get fleet-wide metrics easily
- Ability to auto-tag metadata available at kernel level
- Simplicity of adding profiling at infrastructure vs application level (particularly for large systems)
- []

#### Cons of Kernal profiling

- Requires particular kernel versions
- Constraints on being able to tag user-level code
- Constraints on performant ways to retrive certain kinds of data due to permissions (i.e. Memory profiles)
- Difficulty of developing locally (usually requires Linux)

## Pyroscope's solution: Merge application and kernel profiling

We believe that there's benefits to _both_ eBPF and non-eBPF profiling and our focus long term is to find
ways to combine the two. This is particularly appealing because by combining eBPF profiling with user-space
profilers cancels out all of the negatives leaving us with the best of both worlds.
...
to add a language specific integration if you'd like more granular controls over your profiling data. 

We do this internally for our cloud product. We also use eBPF to profile our rideshare example cluster running on kubernetes for our 
