...
The collective growth is representative of the rapidly growing interest in this space Pyroscope, Pixie, and Parca all gained a significant amount of stars over the same 6 month period.
...
is more frequently being considered a first-class citizen in observability suites. For example, OpenTelemetry has kicked off
...
The negatives of eBPF profiling are the positives of non-eBPF profiling and vice versa. We believe that the best way to get the most value out of profiling
...
### Profiling non-compiled Languages (Ruby, Python, etc.)
When profiling non-compiled languages like Ruby or Python the eBPF profiler (by default) does not have
access to the symbols of the application code.
...
## How to merge user-space and kernel profiling
...
[
  (example) + diagram showing how it would work to do eBPF profiling + ruby profiling / eBPF profiling + python profiling from the kernel
]

Stay tuned for progress on this or better yet let us know if you'd like to help! In the meantime, however, we recommend using:
...
In fact, this is how we profile our cloud product's code internally:
