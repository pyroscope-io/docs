---
title: Experimenting With Memory Arenas in Go 1.20
sidebar_label: Experimenting With Memory Arenas in Go 1.20
...
![go_arenas_1600x900_02](https://user-images.githubusercontent.com/662636/216213933-918be1b3-cd94-4a7f-9d00-519564700486.jpg)
...
In certain scenarios, such as when an HTTP server processes requests with large protobuf blobs, this can result in the Go runtime spending a significant amount of time tracking each individual allocation, and then deallocating them.
At a minimum this casues increased performance overhead and often this can be the root cause of [INSERT BAD THINGS THAT HAPPEN WHEN MEMORY ALLOCATIONS GO WRONG].

Arenas offer a solution to these problems. To reduce overhead in the example of protobuf blobs, a large chunk of memory (an arena) can be allocated before parsing. This reduces the overhead of many small allocations. All parsed objects can then be placed on the arena, and since these objects belong to the arena, they are no longer tracked by the Go runtime, eliminating garbage collection overhead. Once parsing is completed, the entire arena can be freed at once, further reducing the overhead of freeing many small objects.
...
You can see that the majority of allocations (`533.30 M`) come from one area of code — it's the purple one at the bottom where it calls function `InsertStackA`. Given that it represents 65% of allocations this is a good candidate for using arenas. But is it still worthwhile to cut down these allocations? Let's take a look at the CPU profile (`cpu`) of the same service:
...
* the programs spends a lot of time in the same `InsertStackA` function, so we're on the right track.
* if you search for `runtime.mallocgc` (multiple pink nodes at the bottom) you will see that that function is called a lot in various places and it takes about 14% our total execution time.
* we can also see that about 5% of the time is spent in `runtime.gcBgMarkWorker` (pink nodes located in the right section of the flamegraph).
