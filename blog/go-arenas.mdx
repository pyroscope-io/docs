...
<center style={{margin:"10px 20px 20px"}}><i>The largest nodes in this allocated objects flamegraph represent where arenas may be most effective</i></center>
...
<center style={{margin:"10px 20px 20px"}}><i>The largest nodes in this CPU flamegraph represent potential for performance improvements</i></center>

<br/>


A few things stand out:
* The programs spends a lot of CPU time in the same `InsertStackA` function, so there definitely is potential for meaningful performance gains
...
It is hard to see the exact amount of savings from solely looking at the flamegraphs, but when looking at our Grafana dashboard which combines our flamegraph with AWS metrics we get from kubernetes we saw approximately a 8% reduction in CPU usage.
...
And compared to the other types of optimizations, they share the same drawbacks, but provide more benefits — so in our view arenas are a net win.
