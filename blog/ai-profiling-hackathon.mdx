...
This is where many face challenges, particularly beginners. But understanding these flamegraphs is key to simplifying them.
...
It's worth noting that, the flamegraph used for this post is a current representation of our distributor. As you can see, our distributors are struggling with two notorious tasks that most companies are likely not doing perfectly either: compression and regular expressions (fixes comoing soon!). 
...
To try it on your own profiles/flamegraphs you have three options:
1. **Upload to Flamegraph.com:** Upload file containing _one_ flamegraph to [flamegraph.com](http://flamegraph.com) (we will be adding support for multiple profiles soon) and click the "ai explainer" button
2. **Upload to Grafana Cloud:** Send profiling data to [Grafana Cloud Profiles](https://grafana.com/auth/sign-up/create-user?pg=pyroscope-ai-blog) and look at the Cloud profiles app (you can sign up for a free account) and click the "ai explainer" button
3. **Wait a bit:** Wait until we release an open source version of this tool (pending positive feedback from the community)
