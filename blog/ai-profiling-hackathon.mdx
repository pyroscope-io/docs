...
# AI-Powered Flamegraph Interpreter: Our Grafana Hackathon Project
...
If you're of the opinion that AI isn't going to solve all the world's problems, you're probably right. And if you think it could, well, you might be onto something too. I'm not here to spark a debate on when AI will take over the world...Instead, let me walk you through an interesting experiment we tried during the Grafana Hackathon with our project, Pyroscope.

Here at Pyroscope, we've gotten many requests to sprinkle some AI magic into our flamegraphs, both from our team and our users. **AI is really bad a at a lot of things**, but one thing it does excel at, arguably better than humans is language-based tasks. 

Interpreting and communicating the contents of a flamegraph in plain english (or other language) seemed like a perfect opportunity to explore if AI could actually make a positive impact on UX for continuous profiling.

So, when Hackathon week came around, we decided to put our skepticism aside and see exactly how AI stacks up against humans in this notoriously tricky task: flamegraph analysis.

Ultimately our goal this week is to answer the question: **Can AI positively improve the user experience analyzing and interpretating flamegraphs?**
...
These are the three key questions in the (theoretically) simple workflow of interpreting flamegraphs in continuous profilng tools:

1. **Performance Bottleneck**: What's slowing things down?
2. **Root Cause**: Why is this happening?
3. **Recommended Fix**: How can you can fix it?
...
While most of these are clearly just for fun, in reality we can adjust these prompts (or allow users to adjust them) to guide everyone from `beginner` to `expert` through the same flamegraph analysis and provide a tailored experience for each user (it even translates to other languages!). 

## Try It Out on your own flamegraphs (any language) and Share Your Feedback!

It's worth noting that, the flamegraph used for this post is a current representation of our distributor. As you can see are struggling with two notorious tasks that most companies are likely not doing perfectly either: 
- compression
- regular expressions

Even many companies who have adopted continuous profiling still struggle with taking the time to uncover insights like these, but with AI we can releive some of this burden.

However, while AI was successful in analyzing _this particular_ flamegraph, there are probably 51,331,542 more cases where AI fails spectacularly and **we'd love for you to find and share them with us!**
