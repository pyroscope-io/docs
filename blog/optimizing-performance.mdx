---
title: What Happens when you Profile a Profiler
...
While `pprof` is a great format for representing profiling data, but it's not the most optimal format when it comes to storing profiling data. So, we transcode `pprof` into our internal format that we optimized for storage in our [custom storage engine](/docs/storage-design).
...
<center style={{margin:"-15px 20px 20px"}}><i>`FindFunction` and `FindLocation` consume a significant amount of CPU</i></center>


To understand what it taks to "find" `Location` and `Function` we looked up how pprof objects are structured:
...
If you'd like to learn more about how to get started with Pyroscope and learn where your code's bottleneck is, check out the [Pyroscope documentation](/docs/installing-pyroscope-overview).
