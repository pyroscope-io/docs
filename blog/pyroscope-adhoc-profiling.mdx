...
Our goal is to make Pyroscope a one-stop-shop for *all* profiling needs... And that means supporting all languages, all flamegraph formats, continuously profiling a servers, and -- the topic of this post -- quickly profiling an adhoc script.
...
Note: This html export can also be achieved through the file exported dropdown available in the flamegraph toolbar:
![image](https://user-images.githubusercontent.com/23323466/151273431-701f0fe4-4031-467e-8c13-189482771493.png)

This is the simplest way to use Pyroscope adhoc mode and is great for quickly viewing and sharing profiles that are richer than a simple screenshot. 

### Analyzing Pyroscope-generated Adhoc Flamegraphs
While standalone mode is great for many use cases, one of the benefits of running a Pyroscope server is that it also has a built in mechanisms for storage that allow you to store these exported flamegraphs and revisit them later.
...
### Coming soon!
As of right now (Jan 26), this mode is mostly for three main use cases:
...
- Getting a shareable standalone flamegraph that has the same UI and analysis features as the web interface
