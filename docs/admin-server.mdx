...
:::note
This functionality is available starting from version TODO. Make sure to upgrade before you use this.
:::

Pyroscope has an admin server for certain administration tasks. It uses HTTP over [unix domain sockets](https://en.wikipedia.org/wiki/Unix_domain_socket), which means you can interact either via our the `pyroscope admin` command or via the API directly.


## Interacting via the CLI
Our pyroscope CLI has an `admin` subcommand that allows you to interact with the admin server. It even has autocomplete!
...
That happens when the admin server is not up. Check if you have its flag enabled (it should be enabled by default since version TODO).
...
When starting a new instance, we check if the socket exists, and if so, try to ping it.
