...
```
Integration is very simillar to `net/http/pprof`, you just import a new package and it will expose new endpoints `/debug/pprof/delta_heap`, `/debug/pprof/delta_block`, `/debug/pprof/delta_mutex`
```go
