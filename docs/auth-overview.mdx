...
The list of external authentication providers includes:
 ...
 Pyroscope also supports an internal provider as well
 - [internal authentication](auth-internal.mdx)
...
## Generate an API Key for Pyroscope server
...
By default, anyone can send data to Pyroscope Server without any authentication. for more details check [API key docs](api-key-authentication.mdx).
