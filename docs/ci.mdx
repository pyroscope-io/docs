...
Its functionality is available via the `pyro-ci` binary.

The process consists of 3 steps: Setup, Execution and Upload.

## Installation
<!-- TODO: explain how to download the binary -->
The `pyro-ci` binary is available under the [Github Releases page](https://github.com/pyroscope-io/ci/releases).
...
<Tabs
  ...
  ```sh
  pyro-ci go install --applicationName=myapp pkg
  ```
...
```sh
pyro-ci exec --apiKey="MY_API_KEY" -- go test ./...
```
...
## Upload
:::note
This functionality is only available in Pyroscope Cloud
:::
...
The profiles can be visualized in the CI page under Pyroscope Cloud ([`http://pyroscope.cloud/ci`](`http://pyroscope.cloud`)).


<iframe frameBorder="0" width="100%" height="400" src="https://flamegraph.com/share/65842a18-9cca-11ed-9b0e-8269072f689e/iframe?onlyDisplay=flamegraph&colorMode=light"></iframe>

For illustration, above is the profile for `github.com/pyroscope-io/pyroscope` go tests,
which shows that the race condition detector spends a non-negligible amount of time.
