...
We provide a few different ways for you to install Pyroscope Server on [Linux](server-install-linux), [macOS](server-install-macos) or run it in <FontAwesomeIcon color="#0db7ed" icon={faDocker}/> [Docker](docker-guide). To find out about other ways of installing Pyroscope, or to find packages for other CPU architectures (e.g ARM) visit our [Downloads](/downloads) page.
...
[**Pyroscope Agent**](agent-overview) can be run on [Linux](agent-install-linux), [macOS](agent-install-macos), [Windows](agent-install-windows)
