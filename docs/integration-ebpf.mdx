...
* easy installation, only add the agent on your node, as opposed to instrumenting every single library
* have access to kernel-space stacktraces

However it has a few downsides:
* Only works with interpreted languages


Therefore we recommend an hybrid approach for better results: eBPF to profile the node and specific language instrumentation per application.


### Prerequisites

For this to work you'll need
* a [pyroscope server](https://pyroscope.io/docs/installing-pyroscope-overview/#step-1-install-pyroscope-server) that the agent will have access to
...
https://demo.pyroscope.io/?query=ebpf.cpu%7B%7D
