...
pyroscope.configure(
  ...
  oncpu = True # report cpu time only; default is False
  native = True # profile native extensions; default is False
  gil_only = False # only include traces for threads that are holding on to the Global Interpreter Lock; default is False
