...
It generates two kind of files: profiling data that is stored locally in the
pyroscope data directory and can be later analyzed using the pyroscope UI
and standalone HTML files that can be used without a running pyrsocope server.
...
| no-standalone-html | false | all | Disables generating a standalone HTML file(s), only save profiles to pyroscope data directory. |
