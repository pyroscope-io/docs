...
And then pyroscope will start profiling both Cpu and Heap automatically. To prevent this use `{ autoStart: false }` option and then 
call `Pyroscope.startCpuProfiling()` or `Pyroscope.startHeapProfiling()` 
